...
var rootCmd = &cobra.Command{
	Use: "dms",
	Long: `dms is an open-source HTTP server framework and CMS, released under 
...
var runCmd = &cobra.Command{
	...
	Short: "starts the 'dms' HTTP server for the JSON API and or Admin System.",
	Long: `Starts the 'dms' HTTP server for the JSON API, Admin System, or both.
...
var ErrWrongOrMissingService = errors.New("To execute 'dms serve', " +
...
func init() {
	for _, cmd := range []*cobra.Command{runCmd, serveCmd} {
		cmd.Flags().StringVar(&bind, "bind", "localhost", "address for dms to bind the HTTP(S) server")
		cmd.Flags().IntVar(&httpsport, "https-port", 443, "port for dms to bind its HTTPS listener")
		cmd.Flags().IntVar(&port, "port", 8080, "port for dms to bind its HTTP listener")
