...
var upgradeCmd = &cobra.Command{
	Use:   "upgrade",
	Short: "upgrades your project to the current dms version",
...
package by running '$ go get -u github.com/agreyfox/eshop/...'`,
	Example: `$ dms upgrade`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fmt.Println("Changes you made to dms's internal code will be overwritten.")
...
func upgradeDmsProjectDir(path string) error {
	core := []string{
		".gitattributes",
		"LICENSE",
		"dms-banner.png",
	...
	stamp := fmt.Sprintf("dms-%d.bak", time.Now().Unix())
	...
	if dev {
		if fork != "" {
			...
			fmt.Println("Upgrading from 'dms-dev' branch")
		}
	}
	...
	if err != nil {
		...
		fmt.Println(temp)
		fmt.Println("")
		fmt.Println("Manually create a new dms project here and copy those files within it to fully restore.")
		fmt.Println("")
		return err
	}
...
func init() {
	//upgradeCmd.Flags().StringVar(&fork, "fork", "", "modify repo source for dms core development")
	//upgradeCmd.Flags().BoolVar(&dev, "dev", false, "modify environment for dms core development")
