...
func SetUpload(target string, data url.Values) (int, error) {
	...
	err = store.Update(func(tx *bolt.Tx) error {
		b, err := tx.CreateBucketIfNotExists([]byte(DB__uploads))
		if err != nil {
			return err
		}
		...
		b, err = tx.CreateBucketIfNotExists([]byte(DB__contentIndex))
		if err != nil {
			return err
		}
...
func UploadBySlug(slug string) ([]byte, error) {
	...
	err := store.View(func(tx *bolt.Tx) error {
		...
		v := b.Get([]byte(slug))
		if v == nil {
			return fmt.Errorf("no value for key '%s' in DB__contentIndex", slug)
		}
